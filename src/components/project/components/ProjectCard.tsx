...
const ProjectCard = ({
  ...
  const isPortfolio = project.id === "daesungPortfolio";
  const isExpanded = isOpen || (isPortfolio && modalProject);
  ...
  return (
    <motion.li
      ...
      exit={{ width: "auto" }}
      transition={{ duration: 0.6 }}
      style={{ animationDelay: `${inView && "0.5s"}` }}
          ...
          {isOpen && (
            ...
            >
              ...
              {project.site ? (
                ...
                <p className="text-16-extraLight">운영종료</p>
              )}
            </motion.div>
          )}
        </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* 프로젝트 프리뷰 */}
            <PreviewSection id={project.id} preview={project.preview} />
            {/* 사용 기술 */}
