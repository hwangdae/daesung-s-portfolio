  ...
  works: [
    {
      ...
      description:
        ["<strong>React-hook-form과 Zod를 결합해 폼 검증을 스키마 기반으로 자동화</strong>하여, <strong>입력 오류를 예방하고 코드 일관성과 재사용성을 높였습니다.</strong>"],
    },
    {
      title: "토스페이먼츠 결제 연동",
      thinking:
      ...
      description:
        ["<strong>Toss Payments API를 연동하고 비동기로 전달되는 결제 결과를 처리하는 플로우를 구현해, 안정적인 결제 경험을 제공</strong>했습니다."],
    },
    {
      ...
      description:
        ["<strong>목업 데이터를 활용해 쿠폰(정액/정율)과 포인트를 함께 계산할 수 있는 할인 로직을 구현하고, 최종 결제 금액이 실시간으로 반영되도록 설계했습니다.</strong>"],
    },
    {
      ...
      description:
        ["<strong>useState를 활용해 장바구니 상태를 구성하고, 사이즈별 중복 추가 방지 및 수량 조절 로직을 구현</strong>했습니다. 이를 통해 <strong>사용자 입장에서 직관적이고 부드러운 구매 경험을 제공</strong>했습니다."],
    },
  ],
  troubleShooting: [
    {
