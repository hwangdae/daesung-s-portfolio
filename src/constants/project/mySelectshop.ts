  ...
  works: [
    {
      ...
      thinking: [
        "개발 초기에는 <b>코드 복잡도를 줄이기 위해 유저 데이터 안에 리뷰 데이터를 함께 포함하는 구조를 사용했습니다. 하지만 리뷰 수가 늘어남에 따라 데이터가 많아지면서 렌더링 지연이 발생했고, 특히 리뷰 데이터에 이미지가 포함되어 있어 렌더링 비용이 급격히 증가</b>했습니다.",
      ],
      description: [
        "이를 해결하기 위해 <strong>유저 데이터와 리뷰 데이터를 분리하여 관리하고, 무한스크롤 및 windowing 기법을 사용하여 초기 데이터 양을 줄이고, DOM을 최적화 함으로써 렌더링 비용을 절감</strong> 했습니다. 그 결과, <strong>데이터 로딩 효율성과 사용자 경험을 모두 개선</strong>할 수 있었습니다.",
      ],
    },
    {
      title: "이미지 최적화를 통한 성능 개선",
      thinking: [
        "리뷰 플랫폼 특성상 <b>사용자가 직접 이미지를 업로드할 수 있었기 때문에, 고해상도 이미지 업로드 시 페이지 로딩 지연 및 전체 웹 성능 저하가 발생</b>할 우려가 있었습니다.",
      ],
      description: [
        "이를 해결하기 위해 <strong>업로드 시점에서 이미지를 browser-image-compression 라이브러리를 사용해 사전 압축하고, WebP 포맷으로 자동 변환되도록 구현</strong>했습니다. 그 결과, <strong>이미지 평균 용량을 약 73% 절감하면서도 품질 저하 없이 웹페이지 로딩 속도와 Lighthouse SEO 점수를 모두 개선</strong>할 수 있었습니다.",
      ],
      blog: "https://devdaybook.tistory.com/28",
    },
    {
      title: "이미지 업로드 병렬 처리를 통한 속도 최적화",
      thinking: [
        "<b>여러 이미지를 업로드할 경우, 각 이미지를 개별적으로 압축하고 WebP로 변환해야 했기 때문에 업로드 속도가 느려지는 문제</b>가 있었습니다.",
      ],
      description: [
        "이를 해결하기 위해 <strong>Promise.all를 활용하여 이미지 변환과 업로드를 병렬 처리</strong>하도록 구현했습니다. 덕분에 <strong>업로드 속도를 약 3배 이상 개선</strong>할 수 있었습니다.",
      ],
    },
    {
      title: "폰트 최적화를 통한 TBT(Total Blocking Time) 개선",
      thinking: [
        "개발 초기에는 <b>여러 개의 로컬 폰트를 개별적으로 로드하는 구조로 되어 있었기 때문에, 폰트 파일 수가 많아지고 용량이 커지면서 초기 로딩 속도 저하와 TBT(Total Blocking Time) 증가 문제가 발생</b>했습니다. 또한 <b>폰트 로딩 지연으로 인해 텍스트 깜빡임(FOIT) 현상이 발생</b>하며 사용자 경험에도 악영향을 주었습니다.",
      ],
      description: [
        "이를 해결하기 위해 <strong>여러 개의 폰트 파일을 하나의 가변 폰트(Variable Font)로 통합</strong>, <strong>사용하지 않는 문자 집합을 제거하는 서브세팅(Subsetting)을 적용</strong>하고, <strong>주요 폰트를 `preload`로 미리 불러와 렌더링 지연을 줄였습니다.</strong> 이를 통해 <strong>폰트 파일 크기를 약 50% 줄이고, Lighthouse Performance 점수를 54점에서 84점으로 향상시켰으며, 폰트 로딩 지연과 TBT를 모두 개선</strong>할 수 있었습니다.",
      ],
    ...
    },
    {
      ...
      thinking: [
        "기존에는 <b>개발서버의 변경사항을 배포서버에 적용하기 위해 수동으로 진행</b>했지만, 반복적인 빌드, 업로드 과정이 번거롭고 비효율적이라고 생각 했습니다.",
      ],
      description: [
        "이를 해결하기 위해 <strong>GitHub Actions를 이용한 CI/CD 파이프라인을 구축</strong>했습니다. <strong>main 브랜치에 코드가 올라올 경우 자동으로 빌드부터 서비스 재시작이 수행되도록 설정</strong>했습니다. 이를 통해 <strong>배포 시간을 5분에서 2분까지 단축</strong>할 수 있었습니다.",
      ],
