export const NOWAIT_DETAIL = {
  ...
  works: [
    ...
    },
    {
      ...
      thinking:
        `1. 대학교 축제의 주점들은 보통 좁은 공간에 밀집해 있어, 정확한 위치를 표시하기 위해서는 지도 줌 레벨의 한계가 중요했습니다. 이를 고려해 고배율 확대가 가능한 네이버 지도를 선택했으며, 축제가 열리는 캠퍼스 범위를 명확히 보여주기 위해 polygon을 활용해 사용자 경험을 개선했습니다.\n 2. 주점 위치의 정확한 좌표를 수동으로 파악해야 하는 비효율적인 작업이 반복되면서 운영 측면에서도 개선할 필요가 있었습니다.`,
      description:
        "이 문제를 해결하기 위해 지도 상에서 주점 마커를 자동으로 생성하고, 클릭 한 번으로 좌표를 복사할 수 있는 'MapManage' 페이지를 구현했습니다. 이를 통해 주점 위치 등록 과정을 크게 효율화하고 운영 편의성을 높였습니다.",
    },
    {
    ...
    },
    {
      ...
      description:
        "이 문제를 해결하기 위해 JavaScript의 Image 객체를 활용해 애니메이션 이미지를 비동기적으로 로드하고, 로드 완료 전까지는 가벼운 정적 이미지를 fallback 이미지로 표시하도록 구현했습니다. 애니메이션 이미지가 완전히 로드되면 자연스럽게 교체되도록 하여, 공백 구간을 제거하고 화면 완성도를 높여 UX를 개선했습니다.",
    },
    {
