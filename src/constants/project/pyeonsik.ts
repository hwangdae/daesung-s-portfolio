export const PYEONSIK_DETAIL = {
  ...
  works: [
    ...
    },
    {
      ...
      thinking: [
        "총 4단계로 구성된 고객센터 프로세스에서, 마지막 단계에서만 작성 API를 호출하는 구조였습니다. <b>각 단계로 데이터를 전달하기 위해 props가 점점 많아지면서 심각한 props drilling이 발생했고, 컴포넌트 간 결합도가 높아 유지보수가 어려워졌습니다.</b>",
      ],
      description: [
    ...
    },
    {
      ...
      thinking: [
        "마우스로 카드를 스와이프하며 리뷰를 남기는 기능을 구현할 때, <b>사용자가 자연스럽고 몰입감 있게 느낄 수 있는 모션과 조작감을 가장 중요하게 생각했습니다. 단순한 애니메이션이 아니라 실제 물리적인 카드 이동처럼 보이도록 구현하기 위해 react-card-rotate-swiper 라이브러리를 도입</b>했습니다.",
      ],
      description: [
